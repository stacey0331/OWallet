...
<template>
  <div>
    <breadcrumb
      :routes="routes"
      :current="$t('sharedWalletHome.paxMgmt')"
    ...
    <div class="pax-container">
      <div class="pax-header">
        ...
        >
          <a-radio-button value="0">{{
            $t('sharedWalletHome.unprocessed')
          }}</a-radio-button>
          <a-radio-button value="1">{{
            $t('sharedWalletHome.processing')
          }}</a-radio-button>
          <a-radio-button value="2">{{
            $t('sharedWalletHome.completed')
          }}</a-radio-button>
        ...
        <div v-if="status === '1'" class="pax-selectPayer">
          ...
          <a-select
            :options="localCopayers"
            class="select-current"
            @change="handleChangeCurrentSigner"
          ></a-select>
        </div>
      </div>
      <div class="table-btns">
        ...
        <a-button
          type="primary"
          :disabled="
            selectedRowKeys.length < 1 || (status === '1' && !currentSigner)
          "
          v-if="status === '0' || status === '1'"
          @click="handleProcess"
          >{{ $t('pax.toProcess') }}</a-button
        >
      </div>
  ...
  methods: {
    ...
    async handleVarify() {
      ...
      const result = await this.httpService({
        ...
        data,
        url:
          (net === 'TEST_NET' ? PAX_API.TestHost : PAX_API.Host) +
          PAX_API.validateTx,
      });
      const varifyFailedList = [];
      const ethScanHost =
        net === 'TEST_NET' ? PAX_API.EthScanTest : PAX_API.EthScanMain;
    ...
    handleProcess() {
      ...
      this.totalAmount = list
        .reduce((item1, item2) => {
          return (item1.AmountStr
            ? new BigNumber(item1.AmountStr)
            : new BigNumber(item1)
          ).plus(new BigNumber(item2.AmountStr));
        }, 0)
        .toFixed(8);
      this.visible = true;
    },
    ...
    handleProcessOk() {
      ...
      if (this.status === '0') {
        ...
        this.$store.commit('UDPATE_CURRENT_SIGNER', {
    ...
    updateLocalCopayers() {
      ...
      const localCopayers = [];
      dbService.find(
        { type: { $in: ['CommonWallet', 'HardwareWallet'] } },
    ...
    handleChangeCurrentSigner(value) {
      console.log(value);
      this.currentSigner = this.localCopayers.find(
