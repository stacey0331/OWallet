<template>
  <div class="basic-container">
    <p class="confirm-label wallet-label" style="margin-bottom: 40px">
      {{ label }}
    </p>

    <p class="confirm-label font-bold">
      {{ $t('createSharedWallet.copayers') }}({{ copayers.length }})
    </p>
    <div class="confirm-pk-box">
      <div
        class="confirm-pk-item clearfix"
        v-for="(pk, index) in copayers"
        :key="pk.index"
      >
    ...
    <div class="confirm-btns">
      <div class="confirm-btn-container">
        <a-button type="default" class="btn-cancel" @click="back">{{
...
export default {
  ...
  computed: {
    ...
    requiredSigNum: {
      ...
      set(val) {
        this.$store.commit('UPDATE_REQUIRED_SIG_NUMBER', {
          requiredSigNum: val,
        });
      },
    },
  ...
  methods: {
    ...
    next() {
      ...
      const multiAddr = Crypto.Address.fromMultiPubKeys(
        this.requiredSigNum,
        pks
      );
