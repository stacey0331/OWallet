...
<template>
  <div>
    <breadcrumb
      :current="$t('nodeMgmt.stakeHistory')"
      v-on:backEvent="handleRouteBack"
    ></breadcrumb>
    <div class="auth-login">
      <div class="intro-item">
        ...
        <a-radio-group
          ...
          v-model="payerWalletType"
          class="change-payer-radio"
        >
          <a-radio value="commonWallet" class="payer-radio-item">{{
            $t('createIdentity.commonWallet')
          }}</a-radio>
          <a-radio value="ledgerWallet" class="payer-radio-item">{{
...
export default {
  ...
  methods: {
    ...
    handleAuthorizeLogin(record, item) {
      ...
      this.$store.commit('UPDATE_STAKE_AUTHORIZATION_WALLET', {
        stakeWallet: record.stakeWallet,
      });
    ...
    handleChangePayer(value) {
      this.payerWallet = this.normalWallet.find(v => {
    ...
    handleSearch() {
      ...
      this.$store
        .dispatch('searchStakeHistory', { address: this.stakeWallet.address })
        .then(res => {
