  ...
  <div>
    ...
    <div class="top-menu-container" v-if="role.indexOf(VOTE_ROLE.ADMIN) > -1">
      ...
      <div
        class="btn-new"
        @click="handleAddVote"
        v-if="role.indexOf(VOTE_ROLE.ADMIN) > -1"
      ></div>
    </div>
      ...
      <div class="list-item" v-for="item in admin_votes" :key="item.hash">
        ...
        <div class="actions">
          ...
          <a-button
            type="link"
            style="color: #ea4545"
            @click="onStopVote(item)"
            v-if="
              item.statusText === VOTE_STATUS_TEXT.NOT_START ||
              item.statusText === VOTE_STATUS_TEXT.IN_PROGRESS
            "
            >Stop Vote</a-button
          >
        </div>
...
export default {
  ...
  data() {
    ...
    return {
      routes,
      current: ['all'],
      menus: [{ key: 'all', name: this.$t('vote.allVotes') }],
      signVisible: false,
      tx: '',
      VOTE_STATUS_TEXT,
    };
