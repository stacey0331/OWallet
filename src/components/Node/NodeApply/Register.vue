<template>
  <div>
    <div v-if="!registerSucceed">
      <breadcrumb
        :current="$t('nodeApply.newNodeApply')"
        v-on:backEvent="back"
      ></breadcrumb>
      <div class="app-container">
        ...
        <div class="steps-content">
          <div class="register-form" v-show="current === 0">
            <div class="proxy-tip">
              <span>{{ $t('nodeApply.proxyServiceTip') }}</span>
              <span @click="onProxyLink" class="proxy-link">{{
                $t('nodeApply.proxyServiceLink')
              }}</span>
            </div>
            <form>
              <div class="form-item">
                <label class="fields-label" for="stakeWallet">{{
              ...
              <div class="form-item">
                ...
                <a-tabs default-active-key="1" type="card">
                  <a-tab-pane
                    key="1"
                    :tab="$t('nodeApply.selectOperationWallet')"
                  >
                    <a-select
              ...
              <div class="footer-btns">
                <div class="btn-known">
                  <a-button type="primary" class="btn-next" @click="next">{{
                    $t('nodeApply.next')
                  }}</a-button>
                </div>
              </div>
          ...
          <div class="register-confirm" v-show="current === 1">
            <div class="form-item">
    ...
    <div class="success-container" v-if="registerSucceed">
      ...
      <a-button class="btn-next" @click="onComplete">{{
  ...
  methods: {
    ...
    next() {
      ...
      if (!this.operationWallet && !this.operationPk) {
        this.$message.error(this.$t('nodeApply.operationWalletRequired'));
        return;
      }
      if (!this.stakeAmount) {
    ...
    onSelectOperationWallet() {
      ...
      if (this.stakeWallet && (this.operationWallet || this.operationPk)) {
        ...
        if (this.stakeWallet.address === address) {
          this.$message.warning(this.$t('nodeApply.sameWalletNotAllowed'));
    ...
    async onComplete() {
      // 进入节点管理页面且打开信息填写的tab
      const nodePk = this.operationWallet
        ? this.operationWallet
        : this.operationPk;
      ...
      this.$store.commit('UPDATE_STAKE_WALLET', {
        stakeWallet: this.stakeWallet,
      });
