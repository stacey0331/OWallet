...
import { getRestClient, getTokenListUrl } from '../../core/utils';

// Deprecated
const oep4s = localStorage.getItem('oep4s')
  ? JSON.parse(localStorage.getItem('oep4s'))
  : [];
...
const actions = {
  async addOep4Token({ commit, dispatch, state }, { scriptHash, address }) {
    try {
      ...
      if (!res.Result) {
        ...
        const balance = await dispatch('queryOep4Balance', {
          scriptHash,
          address,
          decimal
        });
        ...
        dispatch('queryTxForOep4', {
          address,
          oep4s: [...state.oep4s, newOep4]
        });
        return 'ADD_SUCCESS';
      }
    } catch (err) {
  ...
  async queryTxForOep4({ commit, dispatch }, { address, oep4s }) {
    ...
    axios
      .get(url + '/api/v1/explorer/address/' + address + '/10/1')
      .then(response => {
          ...
          for (const t of txlist) {
            ...
            for (const tx of t.TransferList) {
              ...
              for (let o of oep4s) {
                ...
                if (o.symbol === tx.AssetName || tx.AssetName === 'LCY') {
                  // They give the wrong symbol for token LUCKY
