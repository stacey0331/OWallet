...
const mutations = {
  ...
  DELETE_SHARED_WALLET(state, payload) {
    ...
    const index = normalWallet.findIndex(
      w => w.sharedWalletAddress === payload.address
    );
    normalWallet.splice(index, 1);
...
const actions = {
  async fetchWalletsFromDb({ commit }, pk) {
    try {
      const NormalWalletDocs = await dbFind(dbService, {
        type: 'CommonWallet'
      });
      const SharedWalletDocs = await dbFind(dbService, {
        type: 'SharedWallet'
      });
      const HardwareWalletDocs = await dbFind(dbService, {
        type: 'HardwareWallet'
      });
